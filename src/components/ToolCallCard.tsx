...
export default function ToolCallCard({ call }: { call: ToolCallRecord }) {
  ...
  if (!open) {
    return (
      ...
      >
        {statusIcon}
        <span className="truncate font-mono font-medium">{call.name}</span>
  ...
  return (
    <div className={cn('mt-2 overflow-hidden rounded-xl border bg-muted/50 text-xs', statusColor)}>
      ...
      >
        ...
        <ChevronDown className={cn('h-3.5 w-3.5 flex-none transition-transform rotate-180')} />
      </button>
