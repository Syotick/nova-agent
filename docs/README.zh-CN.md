...
**Nova Agent** —— nova 意为"新星"：这个紧凑、可通读、开箱即用的 AI Agent，是下一代 Agent 工程师的起点。面向在校大学生与转行进入 Agent 开发的开发者——Agent 循环、MCP 工具、Agent Skills、记忆、任务自动化——整个技术栈设计成一个晚上就能读完、跑起来、然后动手改造。没有框架黑盒。

> 让 Agent 运转起来的一切，都在一个小代码库里：Agent 循环、MCP 工具、技能系统、多轮对话、轨迹展示、多 Agent 管理、可视化配置、安全隔离——全部具备且可扩展。
...
- **在校大学生 & 转行开发者**——学习 Agent 的最佳开源项目：从第一行读到最后一行的通读、跑通、调试、改造，一切都装在一个小代码库里
- **自托管爱好者**——本地、私密、厂商中立（任意 OpenAI 兼容模型）、零遥测，Node 22+ 单环境跑通
- **二次开发基线**——加技能 = 丢一个文件夹；加工具 = 写一个 JSON 配置

> 如果你需要久经考验、插件生态齐全的商业产品，请用 Claude Code / Cursor 等——本项目为学习与教学而生，不与它们竞争。
...
- ✅ **终端执行（Codex 模式）**：`run_command` 在工作区执行 shell 命令（npm / git / node / python…），捕获输出、超时自动终止、中断时清理整个进程树——读代码 → 改代码 → 跑构建/测试验证 → 启动项目，全在对话里完成
- ✅ **Vibe 自治循环**：输入目标后点 🚀 Vibe——Agent 自动规划、实现、验证、自愈，多轮循环直到收敛（显式 `[DONE]` 判定），带轮数/时长预算，连续相同失败自动熔断止损；中断时清理运行中的进程
