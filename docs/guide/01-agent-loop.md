...
```mermaid
flowchart LR
  U["用户发消息"] --> A1["① 上下文压缩检查<br/>太长先总结旧的"]
  A1 --> A2["② 用户消息落盘 + 附件注入"]
  A2 --> A3["③ 装配工具<br/>MCP 工具 + 内置工具们"]
  A3 --> A4["④ 拼 system prompt<br/>persona + 技能 + 摘要 + 记忆 + 约束"]
  A4 --> A5["⑤ streamText 多步执行<br/>模型 ↔ 工具，直到回答或步骤用尽"]
  A5 --> A6["⑥ 收集文本 / 工具记录 / token"]
  A6 --> A7["⑦ 落盘 assistant 消息 + 发 done 事件"]
```
...
- [ ] 能对着 `runTurn` 说出一个 turn 的 7 个阶段，并解释为什么"上下文压缩"必须在"用户消息落盘"之前
- [ ] 能说清 `streamText` 帮你完成了哪部分（多步工具循环），你自己又在外围做了哪几件事
- [ ] 能解释中断时为什么是"发散式清理"：abort 流 + 杀命令进程树 + 连带中断子任务
- [ ] 能区分"模型错误"与"用户中断"两种收尾为什么都"不落盘空消息"
- [ ] 动手：`NOVA_AGENT_MAX_STEPS=2` 跑一个需要 5 步的任务，亲眼看到"步骤用完被截断"

> 卡住了？回头读对应小节；做完这 5 条再进 [练习册 01](../exercises/01-agent-loop.md)。
