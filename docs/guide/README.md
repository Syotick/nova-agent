...
|---|---|---|---|---|---|
| [01](01-agent-loop.md) | 入门 | `server/agentLoop.ts` | Agent 循环：一个 agent 怎么转起来 | 无 | 讲清楚"一轮对话里模型↔工具怎么循环"；指出 7 个阶段的顺序与各自目的；说出统一工具注册表（toolRegistry）的角色；说出 `streamText` 帮你做了什么、你在外围做了什么 |
...
| [08](08-vibe.md) | 挑战 | `server/vibe.ts` | Vibe 自治循环：目标驱动 | 01-07 全部 | 解释为什么循环必须由编排层驱动；说明 [DONE] 协议与熔断；说出三重预算分别防什么 |
